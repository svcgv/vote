...
public abstract class CollectionUtils {
	...
	/**
	 * 功能描述：该方法用于把字符串集合中空的字符串对象移除。<br>
	 *         根据isEmpty方法检查字符串是否为空，为空的字符串将被从集合中移除。
	 * @param c 字符串集合
	 * @return 字符串集合（已经去掉了空的字符串集合）
	 * @throws Exception 
	 */
	public static Collection removeEmptyString(final Collection c)
			throws Exception {
		if (isEmpty(c)) {
			return null;
		}
		...
		while (iter.hasNext()) {
			...
			if (o == null) {
				...
				throw new Exception("集合中包含的对象类型为非字符串型！");
			}
		}
