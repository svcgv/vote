...
public class TokenHelper {
	 ...
	 * 保存token值的默认命名空间
	 */
    public static final String TOKEN_NAMESPACE = "hz.tokens";
    
    /**
     * 持有token名称的字段名
     */
    public static final String TOKEN_NAME_FIELD = "hz.token.name";
...
//            redisCacheClient.listLpush(tokenName0, token);
    ...
    public static String getToken(HttpServletRequest request, String tokenName){
        ...
        if((tokens == null) || (tokens.length < 1)){
            System.out.println("Could not find token mapped to token name " + tokenName);
            return null;
        }
    ...
    public static String getTokenName(HttpServletRequest request){
        ...
        if(!params.containsKey(TOKEN_NAME_FIELD)){
        	System.out.println("Could not find token name in params.");
            return null;
        }
        ...
        if((tokenNames == null) || (tokenNames.length < 1)){
        	System.out.println("Got a null or empty token name.");
            return null;
        }
    ...
    public static boolean validToken(HttpServletRequest request){
        ...
        if(tokenName == null){
        	System.out.println("no token name found -> Invalid token ");
            return false;
        }
