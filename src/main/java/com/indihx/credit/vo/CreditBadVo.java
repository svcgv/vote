/**
 * 
 */
package com.indihx.credit.vo;

import com.indihx.system.vo.BaseVo;

/**
 * <p>标    题: 物业管理信息系统(PMS)</p>
 * <p>描    述: CreditBadVo.java</p> 
 * <p>版    权: Copyright (c) 2018  </p>
 * <p>公    司: 上海泓智信息科技有限公司</p>
 * <p>创建时间: 2018年3月12日上午8:39:50</p>
 * <p>@author zhengwei</p>
 * <p>@version 1.0</p>
 * <p>CreditBadVo.java</p>
 * <p></p>
 */
public class CreditBadVo extends BaseVo{

	private static final long serialVersionUID = 1L;
	String credit_code;
	String app_id;
	String credit_status;
	String info_from;
	String bllx;
	String jlrq;
	String jlsj;
	String jldwid;
	String jlczybh;
	String slnr;
	String ckf;
	String jlf;
	String bjlx;
	String gzrq;
	String gzsj;
	String gzdwid;
	String gzczybh;
	String zgqx;
	String zgjzrq;
	String ssqx;
	String ssjzrq;
	String ssbz;
	String gdrq;
	String gdsj;
	String gdczybh;
	String score;
	String remark;
	
	String xmid;
	String xmmc;
	String wygsid;
	String wygsmc;
	String zjbh;
	String gsryid;
	String ryxm;
	String sfzbh;
	String hpbid;
	String hpbmc;
	
	String [] credit_seq;
	String [] jlfs;
	String [] jfsm;
	String [] clyj;
	
	/**
	 * @return the credit_seq
	 */
	public String[] getCredit_seq() {
		return credit_seq;
	}
	/**
	 * @param credit_seq the credit_seq to set
	 */
	public void setCredit_seq(String[] credit_seq) {
		this.credit_seq = credit_seq;
	}
	/**
	 * @return the jlfs
	 */
	public String[] getJlfs() {
		return jlfs;
	}
	/**
	 * @param jlfs the jlfs to set
	 */
	public void setJlfs(String[] jlfs) {
		this.jlfs = jlfs;
	}
	/**
	 * @return the jfsm
	 */
	public String[] getJfsm() {
		return jfsm;
	}
	/**
	 * @param jfsm the jfsm to set
	 */
	public void setJfsm(String[] jfsm) {
		this.jfsm = jfsm;
	}
	/**
	 * @return the clyj
	 */
	public String[] getClyj() {
		return clyj;
	}
	/**
	 * @param clyj the clyj to set
	 */
	public void setClyj(String[] clyj) {
		this.clyj = clyj;
	}
	/**
	 * @return the credit_code
	 */
	public String getCredit_code() {
		return credit_code;
	}
	/**
	 * @param credit_code the credit_code to set
	 */
	public void setCredit_code(String credit_code) {
		this.credit_code = credit_code;
	}
	
	/**
	 * @return the app_id
	 */
	public String getApp_id() {
		return app_id;
	}
	/**
	 * @param app_id the app_id to set
	 */
	public void setApp_id(String app_id) {
		this.app_id = app_id;
	}
	/**
	 * @return the credit_status
	 */
	public String getCredit_status() {
		return credit_status;
	}
	/**
	 * @param credit_status the credit_status to set
	 */
	public void setCredit_status(String credit_status) {
		this.credit_status = credit_status;
	}
	/**
	 * @return the info_from
	 */
	public String getInfo_from() {
		return info_from;
	}
	/**
	 * @param info_from the info_from to set
	 */
	public void setInfo_from(String info_from) {
		this.info_from = info_from;
	}
	/**
	 * @return the bllx
	 */
	public String getBllx() {
		return bllx;
	}
	/**
	 * @param bllx the bllx to set
	 */
	public void setBllx(String bllx) {
		this.bllx = bllx;
	}
	/**
	 * @return the jlrq
	 */
	public String getJlrq() {
		return jlrq;
	}
	/**
	 * @param jlrq the jlrq to set
	 */
	public void setJlrq(String jlrq) {
		this.jlrq = jlrq;
	}
	/**
	 * @return the jlsj
	 */
	public String getJlsj() {
		return jlsj;
	}
	/**
	 * @param jlsj the jlsj to set
	 */
	public void setJlsj(String jlsj) {
		this.jlsj = jlsj;
	}
	/**
	 * @return the jldwid
	 */
	public String getJldwid() {
		return jldwid;
	}
	/**
	 * @param jldwid the jldwid to set
	 */
	public void setJldwid(String jldwid) {
		this.jldwid = jldwid;
	}
	/**
	 * @return the jlczybh
	 */
	public String getJlczybh() {
		return jlczybh;
	}
	/**
	 * @param jlczybh the jlczybh to set
	 */
	public void setJlczybh(String jlczybh) {
		this.jlczybh = jlczybh;
	}
	/**
	 * @return the slnr
	 */
	public String getSlnr() {
		return slnr;
	}
	/**
	 * @param slnr the slnr to set
	 */
	public void setSlnr(String slnr) {
		this.slnr = slnr;
	}
	/**
	 * @return the ckf
	 */
	public String getCkf() {
		return ckf;
	}
	/**
	 * @param ckf the ckf to set
	 */
	public void setCkf(String ckf) {
		this.ckf = ckf;
	}
	/**
	 * @return the jlf
	 */
	public String getJlf() {
		return jlf;
	}
	/**
	 * @param jlf the jlf to set
	 */
	public void setJlf(String jlf) {
		this.jlf = jlf;
	}
	/**
	 * @return the bjlx
	 */
	public String getBjlx() {
		return bjlx;
	}
	/**
	 * @param bjlx the bjlx to set
	 */
	public void setBjlx(String bjlx) {
		this.bjlx = bjlx;
	}
	/**
	 * @return the gzrq
	 */
	public String getGzrq() {
		return gzrq;
	}
	/**
	 * @param gzrq the gzrq to set
	 */
	public void setGzrq(String gzrq) {
		this.gzrq = gzrq;
	}
	/**
	 * @return the gzsj
	 */
	public String getGzsj() {
		return gzsj;
	}
	/**
	 * @param gzsj the gzsj to set
	 */
	public void setGzsj(String gzsj) {
		this.gzsj = gzsj;
	}
	/**
	 * @return the gzdwid
	 */
	public String getGzdwid() {
		return gzdwid;
	}
	/**
	 * @param gzdwid the gzdwid to set
	 */
	public void setGzdwid(String gzdwid) {
		this.gzdwid = gzdwid;
	}
	/**
	 * @return the gzczybh
	 */
	public String getGzczybh() {
		return gzczybh;
	}
	/**
	 * @param gzczybh the gzczybh to set
	 */
	public void setGzczybh(String gzczybh) {
		this.gzczybh = gzczybh;
	}
	/**
	 * @return the zgqx
	 */
	public String getZgqx() {
		return zgqx;
	}
	/**
	 * @param zgqx the zgqx to set
	 */
	public void setZgqx(String zgqx) {
		this.zgqx = zgqx;
	}
	/**
	 * @return the zgjzrq
	 */
	public String getZgjzrq() {
		return zgjzrq;
	}
	/**
	 * @param zgjzrq the zgjzrq to set
	 */
	public void setZgjzrq(String zgjzrq) {
		this.zgjzrq = zgjzrq;
	}
	/**
	 * @return the ssqx
	 */
	public String getSsqx() {
		return ssqx;
	}
	/**
	 * @param ssqx the ssqx to set
	 */
	public void setSsqx(String ssqx) {
		this.ssqx = ssqx;
	}
	/**
	 * @return the ssjzrq
	 */
	public String getSsjzrq() {
		return ssjzrq;
	}
	/**
	 * @param ssjzrq the ssjzrq to set
	 */
	public void setSsjzrq(String ssjzrq) {
		this.ssjzrq = ssjzrq;
	}
	/**
	 * @return the ssbz
	 */
	public String getSsbz() {
		return ssbz;
	}
	/**
	 * @param ssbz the ssbz to set
	 */
	public void setSsbz(String ssbz) {
		this.ssbz = ssbz;
	}
	/**
	 * @return the gdrq
	 */
	public String getGdrq() {
		return gdrq;
	}
	/**
	 * @param gdrq the gdrq to set
	 */
	public void setGdrq(String gdrq) {
		this.gdrq = gdrq;
	}
	/**
	 * @return the gdsj
	 */
	public String getGdsj() {
		return gdsj;
	}
	/**
	 * @param gdsj the gdsj to set
	 */
	public void setGdsj(String gdsj) {
		this.gdsj = gdsj;
	}
	/**
	 * @return the gdczybh
	 */
	public String getGdczybh() {
		return gdczybh;
	}
	/**
	 * @param gdczybh the gdczybh to set
	 */
	public void setGdczybh(String gdczybh) {
		this.gdczybh = gdczybh;
	}
	/**
	 * @return the score
	 */
	public String getScore() {
		return score;
	}
	/**
	 * @param score the score to set
	 */
	public void setScore(String score) {
		this.score = score;
	}
	/**
	 * @return the remark
	 */
	public String getRemark() {
		return remark;
	}
	/**
	 * @param remark the remark to set
	 */
	public void setRemark(String remark) {
		this.remark = remark;
	}
	/**
	 * @return the xmid
	 */
	public String getXmid() {
		return xmid;
	}
	/**
	 * @param xmid the xmid to set
	 */
	public void setXmid(String xmid) {
		this.xmid = xmid;
	}
	/**
	 * @return the xmmc
	 */
	public String getXmmc() {
		return xmmc;
	}
	/**
	 * @param xmmc the xmmc to set
	 */
	public void setXmmc(String xmmc) {
		this.xmmc = xmmc;
	}
	/**
	 * @return the wygsid
	 */
	public String getWygsid() {
		return wygsid;
	}
	/**
	 * @param wygsid the wygsid to set
	 */
	public void setWygsid(String wygsid) {
		this.wygsid = wygsid;
	}
	/**
	 * @return the wygsmc
	 */
	public String getWygsmc() {
		return wygsmc;
	}
	/**
	 * @param wygsmc the wygsmc to set
	 */
	public void setWygsmc(String wygsmc) {
		this.wygsmc = wygsmc;
	}
	/**
	 * @return the zjbh
	 */
	public String getZjbh() {
		return zjbh;
	}
	/**
	 * @param zjbh the zjbh to set
	 */
	public void setZjbh(String zjbh) {
		this.zjbh = zjbh;
	}
	/**
	 * @return the gsryid
	 */
	public String getGsryid() {
		return gsryid;
	}
	/**
	 * @param gsryid the gsryid to set
	 */
	public void setGsryid(String gsryid) {
		this.gsryid = gsryid;
	}
	/**
	 * @return the ryxm
	 */
	public String getRyxm() {
		return ryxm;
	}
	/**
	 * @param ryxm the ryxm to set
	 */
	public void setRyxm(String ryxm) {
		this.ryxm = ryxm;
	}
	/**
	 * @return the sfzhm
	 */
	public String getSfzbh() {
		return sfzbh;
	}
	/**
	 * @param sfzhm the sfzhm to set
	 */
	public void setSfzbh(String sfzbh) {
		this.sfzbh = sfzbh;
	}
	/**
	 * @return the hpbid
	 */
	public String getHpbid() {
		return hpbid;
	}
	/**
	 * @param hpbid the hpbid to set
	 */
	public void setHpbid(String hpbid) {
		this.hpbid = hpbid;
	}
	/**
	 * @return the hpbmc
	 */
	public String getHpbmc() {
		return hpbmc;
	}
	/**
	 * @param hpbmc the hpbmc to set
	 */
	public void setHpbmc(String hpbmc) {
		this.hpbmc = hpbmc;
	}
	
}
