...
<script type="text/javascript">
	...
	function save() {
		...
		jsonStr.infoId = infoId;
		$.indi.ajaxSubmit({url: "${ctx}/apply/linkTopicAndHouseInfo",success:function(data){
			if(data.status == true){
				layer.alert('关联房屋信息成功！',{icon: 1}, function(index){
					window.parent.qryList();//调用父页面查询方法
					$.indi.closePopup();//关闭页面
				});
			}
			else{
				layer.alert('关联房屋信息失败！',{icon: 2});
			}
	...
	function qryList(){
		...
		//$("#infoId").val(obj.val);
		$.indi.loadTableByQry({
			url : "${ctx }/house/getUnlinkHouseInfoAndSelect"
				...
				<div class="form-group">
					...
					<div class="col-xs-3">
						<input class="form-control col-xs-11" type="text" id="voteStartDate" readonly="readonly"
							name="voteStartDate"  value="${topic.voteStartDate} " >
					</div>
					...
					<div class="col-xs-3 ">
							...
							name="voteEndDate"  value="${topic.voteEndDate }" >
					</div>
