...
function zTreeOnSaveEvent(event, treeId, treeNode) {
 ...
 // 保存到已选机构中
 	if(act == "costForm"){
 		...
 		//实施部门
 		// index
		$("#cost-form-hook input[name='orgName']").val(getCheckedOrg.name);
		 console.log(getCheckedOrg.name)
		$("#cost-form-hook input[name='orgId']").val(getCheckedOrg.orgId);
		 console.log(getCheckedOrg.orgId)
 	}
 	if(act == "index"){
 		// index
		$("#cost-index-form input[name='orgName']").val(getCheckedOrg.name);
		$("#cost-index-form input[name='orgId']").val(getCheckedOrg.orgId);
 	}
 	if(act == "edit"){

 		$("#cost-edit-hook input[name='orgName']").val(getCheckedOrg.name);
		$("#cost-edit-hook input[name='orgId']").val(getCheckedOrg.orgId);
 	}
