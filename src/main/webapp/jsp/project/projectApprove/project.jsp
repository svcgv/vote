...
<style>
.projectApprove-project-wrapper .layui-form-label{width:70px!important;}
</style>
<div style="margin-top:10px;" class="projectApprove-project-wrapper">
	<form class="layui-form" id="projectApprove-query-form" action="">
...
layui.use(['layer', 'form','laydate','table'], function(){
  var layer = layui.layer ,
	...
	var queryParams=$("#projectApprove-query-form").serializeObject();
  // table render
  table.render({
	    elem: '#projectTable',
	   // url: '/vote/pmprojectinfo/list',
	    ...
	    page: true
	    ,data:[{projectId:123,projectName:"asdn"}]
	  });
  var act="${act}";
	console.log(act,'project')
	// 保存 事件
	var win=$(".projectApprove-project-wrapper").getWindow();
	$(".projectApprove-project-wrapper").on("click","#save-hook",function(){
		// 遍历选中的radio
		$(".projectApprove-project-wrapper .layui-table-body table.layui-table tbody tr").each(function(){
			...
			if(isChecked){
				...
				if(act == "index"){
					$("#projectApprove-index-form input[name='projectName']").val(projectId);
					$("#projectApprove-index-form input[name='wbs']").val(projectName);
			 	}else if(act == "form"){
					$("#projectApprove-index-form input[name='wbs']").val(projectId);
					$("#projectApprove-index-form input[name='projectName']").val(projectName);
			 	}
	...
	// 关闭按钮
	var win=$(".projectApprove-project-wrapper").getWindow();
	$(".projectApprove-project-wrapper").on("click","#close-hook",function(){
		win.close();
	});
	...
	* 客户查询按钮
	*/
	$("#projectApprove-query-form #userQuery").click(function(){
		
		var queryParams=$("#projectApprove-query-form").serialize();
		 var newparam = {}
