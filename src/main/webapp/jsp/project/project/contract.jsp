            ...
            $.ajax({
                ...
                success: function(res){
                    ...
                    table.render({
                        ...
                        cols: [[
                            {type: 'checkbox' },
                            {field:'contractCode', title:'合同编号', templet:function(d){
                                var jsonStr = JSON.stringify({"contractId":d.contractId,"contractCode":d.contractCode,"contractName":d.contractName});
                                return '<div class="jsonData" dataStr='+jsonStr+'>'+d.contractCode+'</div>'
                            } },
                ...
                if(obj.checked==true){
                    if(obj.type=='one' && count!=0){// 选中一个
                        ...
                        for(var i=0;i<tempArray.length;i++){
                            if($.inArray(tempArray[i].contractId, ids) == -1) {
                                ids.push(tempArray[i].contractId);
                            }
                        }
                        console.log(ids);
                    }
                    ...
                    if(obj.type=='one'){ // 取消选中的
                        ...
                        for(var j=0;j<tempArray.length;j++){
                            if($.inArray(tempArray[j].contractId, ids) != -1){
                                ids.remove($.inArray(tempArray[j].contractId, ids));
                            }
            ...
            $(".projectGroup-project-wrapper").on("click","#save-hook",function(){
                ...
                for(var i=0;i<ids.length;i++){
                    for(var j=0;j<testData.length;j++){
                        if(ids[i]==testData[j].contractId){
                ...
                for(var k=0;k<tempArray.length;k++){
                    var flag= true;
                    for(var j in chosedContractProject){
                        var proId=chosedContractProject[j].contractId;
                        if(tempArray[k].contractId == proId){
                            flag= false;
                            continue;
                        }
                    }
                    if(flag){
                    	chosedContractProject.push(tempArray[k]);
                    }

                }

                console.log(chosedContractProject)
