...
<script type="text/javascript">
	...
	function save() {
		...
		$.indi.ajaxSubmit({url: "${ctx}/checktheme/saveEditThemeInfo",success:function(data){
			if(data.status == true){
				layer.alert('检查主题信息变更成功！',{icon: 1}, function(index){
					window.parent.qryList();
					$.indi.closePopup();
				});  
			}else{
				layer.alert('检查主题信息变更失败！',{icon: 2});
			}
			}});
	}
			...
			<form method="post" id="i-form" class="form-horizontal">
			  ...
			  <input type="hidden" id="theme_id" name="theme_id" value="${theme.theme_id}"/>
				...
				<div class="form-group">
					...
					<div class=" col-xs-3">
							<input type="text" class="form-control col-xs-11"
							placeholder="检查开始日期" id="start_date" onClick="WdatePicker()"  name="start_date" value="${theme.start_date}" minlength="2" maxlength="40" />
					</div> 
					<label class="col-xs-2 control-label text-right">检查结束日期</label>
					<div class=" col-xs-3"> 
							<input type="text" class="form-control col-xs-11"
							placeholder="检查结束日期" id="end_date"   onClick="WdatePicker()" name="end_date" value="${theme.end_date}" minlength="2" maxlength="40" />
					</div>  
				</div> 
	 			<div class="form-group">
				...
				<div class="form-group">
					...
					<div class=" col-xs-7">
						<input type="text" class="form-control col-xs-10"
							placeholder="清选择检查的项目" id="sect_name_arr" name="sect_name_arr"   value="${sect_name_arr}"  maxlength="80" readonly="readonly"/>
					</div>
					<button type="button" class="btn btn-primary" onclick="openSect()">
					<i class="icon-plus-sign"></i><span>选择</span></button>
					<input type="hidden" value="${sect_id_arr}" name="sect_id" id="sect_id_arr" />
				</div>
				...
				<div class="form-group">
					...
					<div class=" col-xs-8">
						<textarea type="text" class="form-control col-xs-11"  id="remark" name="remark" rows="5" maxlength="512">${theme.remark}</textarea>
					</div>
				</div>
				<div class="form-group">
