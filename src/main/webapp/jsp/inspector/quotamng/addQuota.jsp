...
<script type="text/javascript">
	...
	function save() { 
		$.indi.ajaxSubmit({url: "${ctx}/quotamng/saveQuotaInfo",success:function(data){
			if(data.status == true){
				layer.alert('检查指标信息新增成功！',{icon: 1}, function(index){
					window.parent.qryList();
					$.indi.closePopup();
				});  
			}else{
				layer.alert('检查指标信息新增失败！',{icon: 2});
			}
		}});
	}
	//关闭当前弹出框
		...
		<div class="row">
			<form method="post" id="i-form" class="form-horizontal">
				<div class="form-group">
					...
					<div class=" col-xs-8">
						<input type="text" class="form-control col-xs-11" disabled="disabled" placeholder="系统自动生成" id="zbbm" name="zbbm" />
					</div>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<div class=" col-xs-8">
						<input type="text" class="form-control col-xs-11" placeholder="指标名称" id="zbmc" name="zbmc" />
					</div>
				</div>
				...
				<div class="form-group" id="d2" style="display: none">
					...
					<div class=" col-xs-8">
						<select class="col-md-8 form-control" id="super_check_seq"  name="super_check_seq" >
							<option>--请选择--</option>
							<c:forEach items="${list}" var="list" varStatus="listSta">
							<option name="jdid" value="${list.check_seq}">${list.zbmc}</option>
							</c:forEach>
						</select>
					</div>
				</div>
				...
				<div id="d1" style="display: none">
					<div class="form-group">
						...
						<div class=" col-xs-8">
							<textarea type="text" class="form-control col-xs-11" placeholder="考核指标细则" id="jfyj" name="jfyj" rows="10"></textarea>
						</div>
					</div>
					<div class="form-group">
						<label class="col-xs-2 control-label text-right">条</label>
						<div class=" col-xs-3">
							<input type="text" class="form-control col-xs-11" placeholder="条" id="tseq" name="tseq"/>
						</div> 
						<label class="col-xs-2 control-label text-right">款</label>
						<div class=" col-xs-3">
							<input type="text" class="form-control col-xs-11" placeholder="款" id="kseq" name="kseq"/>
						</div>
					</div>
					<div class="form-group">
						...
						<div class=" col-xs-3">
							<input type="text" class="form-control col-xs-11" placeholder="分值，范围1至20" id="ckfz" name="ckfz"/>
						</div>
					</div>
				</div>
