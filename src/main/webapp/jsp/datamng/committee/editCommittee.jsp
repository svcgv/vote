...
<script type="text/javascript">
	function save() {
		...
		$.indi.ajaxSubmit({url: "${ctx}/cmt/editCommitteeInfo",success:function(data){
			if(data.status == true){
				layer.alert('社区修改成功！',{icon: 1}, function(index){
					window.parent.qryList();
					$.indi.closePopup();
				});  
			}else{
				layer.alert('社区修改失败！',{icon: 2});
			}
			}});
	}
	//关闭当前弹出框
				...
				<div class="form-group">
					...
					<div class=" col-xs-6">
						<input type="number" class="form-control col-xs-11" 
						 id="sqbm" name="sqbm" value="${info.sqbm}" minlength="4" maxlength="4"/>
					</div>
				</div>
				<div class="form-group">
					...
					<div class=" col-xs-6">
							<input type="text" class="form-control col-xs-11" 
							 id="sqmc" name="sqmc" value="${info.sqmc}"/>
					</div>
				</div>
				<div class="form-group">
					...
					<div class=" col-xs-6">
						<select class="col-md-8 form-control" name="jdid" required >
							...
							<option name="jdid" value="${list.jdid }" <c:if test='${info.jdid == list.jdid}'>selected="selected"</c:if>>
								${list.jdmc}
							</option>
							</c:forEach>
						</select>
